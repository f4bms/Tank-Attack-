...
public:
    ...
    static std::vector<QPoint> bfsPath(const Map& gameMap, int startRow, int startCol, int targetRow, int targetCol) {
        ...
        int numRows = gameMap.getNumRows();
        int numCols = gameMap.getNumCols();

        std::vector<std::vector<bool>> visited(numRows, std::vector<bool>(numCols, false));
        std::vector<std::vector<QPoint>> previous(numRows, std::vector<QPoint>(numCols, QPoint(-1, -1))); // Fixed initialization
        std::queue<QPoint> queue;

        queue.push({startRow, startCol});
        visited[startRow][startCol] = true;

        std::vector<QPoint> directions = {{0, 1}, {1, 0}, {0, -1}, {-1, 0}}; // Right, Down, Left, Up
        ...
        while (!queue.empty()) {
            ...
            if (current.x() == targetRow && current.y() == targetCol) {
                ...
                for (QPoint at = {targetRow, targetCol}; at.x() != -1 && at.y() != -1; at = previous[at.x()][at.y()]) {
                    path.push_back(at);
                }
            ...
            for (const QPoint& dir : directions) {
                ...
                // Check validity and whether it's not an obstacle
